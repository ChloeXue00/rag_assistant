         ...
         world_percept_assig::UpdateObjectList::Response &res)
{
    ...
    ROS_INFO_STREAM("Object Pose:( "<<req.object_pose.position.x << "," 
    ...
    for (size_t i = 0; i < map_objs_.size(); i++)
    {
        ...
        ROS_INFO_STREAM(
            "Object Name: " << map_objs_[i].first 
                    << ", Position: ("  << map_objs_[i].second.position.x << ", " 
                    << map_objs_[i].second.position.y << ", " 
                    << map_objs_[i].second.position.z << ")"
        );
    }
...
void tf_timer_callback(const ros::TimerEvent& e)
{
    ...
    for (size_t i = 0; i < map_objs_.size(); i++)
    {
        ...
        tf.setOrigin(tf2::Vector3(object_pose.position.x , object_pose.position.y, object_pose.position.z)); 

        //TODO: use the correct variable to define the right object orientation (0.5 pts)
        tf.setRotation(tf2::Quaternion(object_pose.orientation.x,object_pose.orientation.y,object_pose.orientation.z,object_pose.orientation.w)); 
